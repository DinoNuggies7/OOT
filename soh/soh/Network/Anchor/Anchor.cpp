...
void Anchor::Enable() {
    Network::Enable(CVarGetString(CVAR_REMOTE_ANCHOR("Host"), "anchor.proxysaw.dev"), CVarGetInteger(CVAR_REMOTE_ANCHOR("Port"), 43384));
...
void Anchor::DrawMenu() {
    ...
    uint16_t port = CVarGetInteger(CVAR_REMOTE_ANCHOR("Port"), 43384);
        ...
        if (isConnected) {
            ...
            if (roomState.ownerClientId == ownClientId) {
                if (ImGui::BeginMenu("Room Settings")) {
                    ...
                    if (UIWidgets::EnhancementCombobox(CVAR_REMOTE_ANCHOR("RoomSettings.pvpMode"), pvpModes, 0)) {
            ...
            ImGui::Text("%s%s", CVarGetString(CVAR_REMOTE_ANCHOR("Name"), ""), IsSaveLoaded() ? (" - " + SohUtils::GetSceneName(gPlayState->sceneNum)).c_str() : "");
            for (auto& [clientId, client] : Anchor::clients) {
                if (client.self) {
                    continue;
                }

                std::string location = " - " + SohUtils::GetSceneName(client.sceneNum);
                if (!client.online) {
                    location = " - offline";
